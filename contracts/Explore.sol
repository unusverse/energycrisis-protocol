...
interface IDiscount {
    function discount(address _user) external view returns (uint256);
}
...
contract Explore is OwnableUpgradeable, PausableUpgradeable {
    ...
    event ExploreResult(address indexed account, uint8 payToken, uint8 cid, uint256 capacity);
    ...
    function explore(uint8 _payToken, uint8 _cid) external payable whenNotPaused {
        ...
        uint256 off = (address(discount) == address(0)) ? 100 : discount.discount(_msgSender());
        if (off > 0) {
        ...
        if (c.maxCapacityBuf == c.minCapacityBuf) {
            ...
            uint8 capacityBuf = c.minCapacityBuf + uint8(r % (c.maxCapacityBuf - c.minCapacityBuf));
        ...
        minted++;
        cidToMinted[_cid] += 1;
        emit ExploreResult(msg.sender, _payToken, _cid, c3[0]);
    }
